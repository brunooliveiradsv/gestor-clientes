document.addEventListener('DOMContentLoaded', () => {
    ...
    const validarCPF = (cpf) => { cpf = String(cpf).replace(/[^\d]+/g, ''); if (cpf.length !== 11 || /^(\d)\1+$/.test(cpf)) return false; let soma = 0; let resto; for (let i = 1; i <= 9; i++) soma += parseInt(cpf.substring(i - 1, i)) * (11 - i); resto = (soma * 10) % 11; if ((resto === 10) || (resto === 11)) resto = 0; if (resto !== parseInt(cpf.substring(9, 10))) return false; soma = 0; for (let i = 1; i <= 10; i++) soma += parseInt(cpf.substring(i - 1, i)) * (12 - i); resto = (soma * 10) % 11; if ((resto === 10) || (resto === 11)) resto = 0; if (resto !== parseInt(cpf.substring(10, 11))) return false; return true; };
    
    // --- Funções de API e UI ---
    const getClientes = async () => { /* ... */ try { const response = await fetch(`${API_URL}/clientes`); if (!response.ok) throw new Error('Erro de rede.'); todosOsClientes = await response.json(); return todosOsClientes; } catch (error) { console.error("Erro ao buscar clientes:", error); showAlert('Erro', 'Não foi possível carregar os clientes.'); return []; } };
    ...
    const renderizarTabela = async (usarCache = false) => { /* ... (continua a mesma) ... */
        ...
        clientes.sort((a, b) => { const valA = a[sortState.key] || ''; const valB = b[sortState.key] || ''; return sortState.order === 'asc' ? valA.localeCompare(valB, 'pt-BR') : valB.localeCompare(valA, 'pt-BR'); });
        ...
        infoPagina.textContent = `Página ${currentPage} de ${totalPages || 1}`; btnAnterior.disabled = currentPage === 1; btnProxima.disabled = currentPage === totalPages || totalPages === 0;
    ...
    const prepararEdicao = async (id) => {
        ...
        try {
            ...
            showAlert('Erro', 'Não foi possível carregar os dados do cliente para edição.');
        }
    };
    ...
    const salvarCliente = async (event) => {
        ...
        try {
            ...
            resetarFormulario();
            showAlert('Sucesso!', `Cliente ${isEditing ? 'atualizado' : 'salvo'} com sucesso.`);
            renderizarTabela();
    ...
    const removerCliente = async (id) => { /* ... (continua a mesma) ... */
        showConfirm('Confirmar Remoção', 'Tem certeza que deseja remover este cliente?', async () => {
            try {
                const response = await fetch(`${API_URL}/clientes/${id}`, { method: 'DELETE' });
                if (!response.ok) { throw new Error('Falha ao remover o cliente.'); }
