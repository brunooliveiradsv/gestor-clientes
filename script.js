document.addEventListener("DOMContentLoaded", () => {
  ...
  const clearAllErrors = () => {
    form.querySelectorAll(".error").forEach((input) => clearError(input));
  };
  ...
  const processarDadosImportados = (novosClientes) => {
    ...
    showAlert(
      ...
      `${importadosComSucesso} cliente(s) importado(s). ${ignorados} registro(s) ignorado(s) (inválidos ou duplicados).`
    );
  };
  ...
  const prepararEdicao = (cpf) => {
    clearAllErrors();
    const cliente = getClientes().find((c) => c.cpf === cpf);
    if (cliente) {
      ...
      cpfEscondidoInput.value = cliente.cpf;
  ...
  const salvarCliente = (event) => {
    ...
    clearAllErrors();
    let isValid = true;
    const { nome: nomeInput, cpf: cpfInput, email: emailInput } = form.elements;
    ...
    const cpfOriginal = cpfEscondidoInput.value;
    if (!cpfOriginal) {
      if (!validarCPF(cpf)) {
        showError(cpfInput, "O CPF inserido não é válido.");
        isValid = false;
      } else if (getClientes().some((c) => c.cpf === cpf)) {
        showError(cpfInput, "Já existe um cliente com este CPF!");
        isValid = false;
      }
    }
    if (!isValid) return;
    ...
    if (cpfOriginal) {
      const index = clientes.findIndex((c) => c.cpf === cpfOriginal);
  ...
  const removerCliente = (cpf) => {
    const aoConfirmar = () => {
      let clientes = getClientes().filter((cliente) => cliente.cpf !== cpf);
      setClientes(clientes);
      renderizarTabela();
      showAlert("Operação Concluída", "O cliente foi removido.");
    };
    showConfirm(
      "Confirmar Remoção",
  ...
  btnProxima.addEventListener("click", () => {
    const totalRows = getClientes().filter((c) =>
      filtroInput.value
  ...
  tabelaCorpo.addEventListener("click", (event) => {
    ...
    if (btnEditar) prepararEdicao(btnEditar.dataset.cpf);
    if (btnRemover) removerCliente(btnRemover.dataset.cpf);
  });
